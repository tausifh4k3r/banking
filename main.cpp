...
p:
            ss << "UPDATE banking SET name = '" + new_name + "' WHERE account_no = '" + acno + "'";
            string query = ss.str();
            const char * q = query.c_str();
            mysql_query(conn, q);
...
sa:
            ss << "UPDATE banking SET phoneno = '" + mob + "' WHERE account_no = '" + acno + "'";
            string query = ss.str();
            const char * q = query.c_str();
            mysql_query(conn, q);
...
string bank::update_email(MYSQL * conn)
{
    ...
    if (x > 0)
    {
        ...
        {
sg:
            cout << "YOUR OLD EMAIL ID IS:" << row[3];
            string email;
...
as:
            ss << "UPDATE banking SET email = '" + email + "' WHERE account_no = '" + acno + "'";
            string query = ss.str();
            const char * q = query.c_str();
            mysql_query(conn, q);
